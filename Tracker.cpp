    ...
    switch (state) {
        case IDLE:
            ...
            if (currentTime - lastAdjustmentTime >= adjustmentPeriodMs) {
                if (filteredBrightness >= brightnessThresholdOhms) {
                    ...
                    terminal.logAdjustmentSkippedLowBrightness((int32_t)filteredBrightness, brightnessThresholdOhms);
                    lastAdjustmentTime = currentTime;
                } else {
                    state = ADJUSTING;
                    lastSamplingTime = currentTime;
        ...
        case ADJUSTING:
            ...
            // Check if it's time to sample sensors
                ...
                int32_t tolerance = (int32_t)(lowerValue * tolerancePercent / 100.0f);
